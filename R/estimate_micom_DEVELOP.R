...
source(file = "R/library-micom_DEVELOP.R") #necessary for computation and permutations
...
evaluate_micom_DEVELOP <- function(original_model, viable_segments, nperms){
  ...
  labels <- data_manipulation(data, original_model, viable_segments)
  ...
  observed_composites <- compute_composite_scores(data = standard_data, ##########
...
micom_results_DEVELOP <- evaluate_micom_DEVELOP(original_model = utaut_model, ####### choose modeB or modeA
                                viable_segments = utaut_data$PERF4 < 5, 
                                nperms = 1000)

micom_reports_DEVELOP <- report_micom(micom_results = micom_results_DEVELOP, confidence_level = 0.95)
